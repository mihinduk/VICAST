...
**Pathway 1** checks if genomes already exist in SnpEff databases, avoiding redundant work, but allowing overwriting by the user to update the annotation. **Pathway 2** processes well-annotated GenBank files by parsing CDS features and mat_peptides for polyproteins and integrating them into SnpEff with viral-optimized settings (5' and 3' UTR extensions, shifted reading frames). **Pathway 3** uses BLASTx homology searches against custom or public protein databases to annotate poorly-characterized genomes, producing curated TSV files for manual review. **Pathway 4** handles segmented viruses (e.g., influenza, rotavirus) by concatenating segments with unique identifiers and managing multi-FASTA references.
...
Following annotation, a four-step post-processing module generates consensus and variant genome sequences. Tier 1 produces a consensus genome by applying variants above a genome-type-aware allele frequency threshold (≥0.95 for haploid ssRNA/ssDNA genomes, ≥0.45 for diploid-like dsRNA/dsDNA), with low-coverage positions (default: <20×) masked as ambiguous bases, and translates per-gene protein sequences using gene coordinates auto-extracted from the SnpEff database. Tier 2 identifies minor variants within a user-defined allele frequency range (default: 0.5–5%), uses BAM-based read co-occurrence analysis to determine physical linkage between variants, and generates one variant genome per linked group applied atop the consensus background, enabling reconstruction of low-frequency haplotypes supported by direct sequencing evidence.
