...
Docker-compatible version with persistent config and environment variable support.
"""
...
# Set Entrez email from environment variable (Docker-friendly)
...
def host_path(container_path):
    """Convert container path to host path for user-facing output."""
    host_dir = os.environ.get('HOST_DIR', '')
    if host_dir and container_path.startswith('/data/'):
        return os.path.join(host_dir, container_path[6:])
    elif host_dir and container_path == '/data':
        return host_dir
...
def combine_fastas(fasta_files, output_fasta, segment_names=None):
    ...
    print(f"  Combined {len(sequences)} sequences -> {host_path(output_fasta)}")
        ...
        for feature in record.features:
            ...
            if feature.type in ["CDS", "gene", "3'UTR", "5'UTR", "mat_peptide"]:
                ...
                if is_compound and feature.type in ["CDS", "mat_peptide"]:
                    ...
                    # Build attributes - prioritize gene name for ID
                    attributes = []
                    if gene_name:
...
def gff_to_tab_delimited(gff_file, output_tsv, segment_to_accession=None):
    ...
    print(f"\nCreated annotation TSV: {host_path(output_tsv)}")
...
def create_custom_snpeff_config(snpeff_data_dir, snpeff_home):
    ...
    Same pattern as step2_add_to_snpeff.py.
    """
    ...
    if not os.path.exists(config_file):
        ...
        with open(config_file, 'w') as f:
            ...
            for line in codon_lines:
                f.write(line)
            f.write("\n# Genome entries (added by VICAST)\n")
...
def add_to_snpeff(genome_id, fasta_file, gff_file, snpeff_data_dir=None, validate=True,
    ...
    Args:
        ...
        snpeff_data_dir: SnpEff data directory
        validate: Whether to attempt CDS/protein validation first
    ...
    if not snpeff_home:
        print("ERROR: SNPEFF_HOME not set. Please set the environment variable.")
        return False

    # Resolve data directory: arg > env var > default
    ...
    # Build database with auto-retry pattern from step2
...
After successful completion:
  snpeff -c /path/to/snpEff.config rvfv variants.vcf > annotated.vcf
    ...
    parser.add_argument('genome_id',
                       help='Genome identifier for SnpEff database (e.g., rvfv, influenza_pr8)')
    parser.add_argument('--segments',
                       help='Comma-separated list of NCBI accessions (e.g., NC_014395.1,NC_014396.1,NC_014397.1)')
    parser.add_argument('--names',
                       help='Comma-separated list of segment names (e.g., L,M,S)')
    ...
    parser.add_argument('--no-validate', action='store_true',
                       help='Skip CDS/protein validation in SnpEff build')
    parser.add_argument('--data-dir',
                       help='SnpEff data directory (default: auto-detect from SNPEFF_DATA)')
    ...
    if not args.no_review:
        ...
        print(f"   {host_path(combined_tsv)}")
    ...
    if snpeff_data_dir:
        ...
        if os.path.exists(snpeff_bin) and not args.update:
            print(f"\nERROR: Genome {args.genome_id} already exists in SnpEff database")
            print(f"  {genome_dir}")
    ...
    if success:
        ...
        # Show host-friendly paths
        config_path = os.path.join(
            snpeff_data_dir or os.environ.get('SNPEFF_DATA', ''),
            'snpEff.config'
        )

        print(f"\nTo test the database:")
        print(f"  snpeff dump -c {config_path} {args.genome_id} | head")
        print(f"\nUsage:")
        print(f"  snpeff -c {config_path} {args.genome_id} variants.vcf > annotated.vcf")
